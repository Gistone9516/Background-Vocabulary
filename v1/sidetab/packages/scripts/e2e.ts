...
try {
  const p1 = await pipeline.classify({ raw_input: "로봇 팔 PID 제어 튜닝을 하려고 한다" }, "ko");
...
try {
  const p2 = await pipeline.nextBranch({ domain: "robotics", job_type: ["문제해결"], history: [{ label: "정확도·품질 높이기", action: "선택" }] }, "ko");
...
try {
  const p5 = await pipeline.detail({ term: "적분기 와인드업", kind: "현상", area: "로봇 제어", job_type: ["문제해결"], domain: "robotics", topic: "PID 제어 튜닝", locale: "en" }, "paid", "ko");
...
try {
  ...
  const r2 = await drain(pipeline.recommendStream(input2, "paid", "ko"));
...
try {
  ...
  const c = await pipeline.classify({ raw_input: raw }, "ko");
