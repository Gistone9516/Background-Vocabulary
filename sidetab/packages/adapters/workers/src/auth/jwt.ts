// 자체 발급 JWT. HS256 으로 서명하며 WebCrypto 만 쓴다(jose 같은 외부 의존 없음).
// 이식성 경계 0-1 이 Web Crypto 를 허용하므로 의존성을 늘리지 않는다. 정본 계약은 인터페이스계약 8장이다.
// 액세스 토큰은 15분, 리프레시 토큰은 30일이다. 시각은 epoch 초로 다룬다(JWT 표준).

import type { AccessTokenClaims, RefreshTokenClaims, Tier } from "@sidetab/shared";
...
// 토큰을 검증한다. 후보 시크릿(현재 키와 이전 키)으로 차례로 맞춰 키 로테이션을 흡수한다.
// 서명과 exp 만 본다(per-request networkless). 반환은 검증된 클레임이거나 null 이다.
...
export async function issueTokens(
  args: { userId: string; tier: Tier; email: string },
  secret: string,
  kid: string
...
// 액세스 토큰을 검증한다. 서명과 exp 만 본다(networkless). 유효하면 클레임, 아니면 null 이다.
...
// 리프레시 토큰을 검증한다. 유효하면 클레임, 아니면 null 이다.
