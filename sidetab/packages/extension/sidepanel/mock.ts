...
export async function relate(input: RelateIn): Promise<RelateOut> {
  await delay(500);
  // DEV: kept가 충분하면 대표 연결 질문, 부족하면 relevant=false로 스킵.
  if (input.kept.length < 3) return { relevant: false, question: "", choices: [], related_terms: [] };
  const terms = input.kept.slice(0, 2).map((k) => k.term);
  return {
    relevant: true,
    question: `이미 담은 ${terms.join(", ")}와 지금 작업이 이어지는 것 같아요. 어느 쪽으로 가시나요?`,
    choices: [
      { label: `${terms[0]}를 실제 작업에 적용하기` },
      { label: `${terms[0]} 위에 더 깊은 개념 쌓기` },
    ],
    related_terms: terms,
  };
}
