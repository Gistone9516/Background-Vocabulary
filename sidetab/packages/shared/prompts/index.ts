...
import { JOB_TYPES, type JobType, type GapType, type Tag } from "../types.js";
...
export function buildPrompt1(raw_input: string, context_object?: string): Msg[] {
  const sys = [
...
export function buildPrompt3(input: {
  ...
  const user = [
    ...
    "이 상황 최우선 어휘 5개에서 8개를 우선순위 순으로 json으로 추천하라.",
  ]
    .filter(Boolean)
    .join("\n");
  return [
...
export function buildPrompt4(input: {
  ...
  const sys = [
    ...
    'paste_text 고정 템플릿: "나는 [분야] 영역에서 [task_intent]를 하려 한다. (내 상황: [user_condition]) 핵심어 A·B·C 중 B는 잘 모른다. (참고 맥락: [context_object])". 빈 슬롯은 생략한다.',
