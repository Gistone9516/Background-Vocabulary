...
// 결제 인증 엔타이틀먼트 (Phase 0). 정본은 인터페이스계약 8장이다.
...
// 결제대행사 식별자. 리터럴 union을 쓰지 않는다(어댑터를 추가해도 이 파일을 안 고치려고). 허용목록 검증은 어댑터에서만 한다.
export type PgProvider = string;

// 결제 통화.
export type Currency = "KRW" | "USD";

// 사용자 레코드. D1 users 행의 도메인 표현이다. PK는 내부 user_id이고 소유 식별자는 email이다.
// google_sub는 소셜 연결 보조 컬럼이며 단독 PK로 쓰지 않는다(계정 삭제 시 고아 구독 회피).
// 시각 필드는 전부 epoch 밀리초다(JWT의 초 단위와 구분).
...
// 엔타이틀먼트. 서버가 판정한 유효 권한이며 JWT에 싣는 부분집합이다.
// grace 기간에는 pro를 유지한다. effective_tier는 tier가 paid거나 grace_until이 현재보다 미래면 paid, 아니면 free다.
...
// 자체 발급 액세스 토큰 클레임. HS256으로 서명한다. 시각은 epoch 초다(JWT 표준).
export interface AccessTokenClaims {
  sub: string; // 내부 user_id
  tier: Tier; // 서버가 판정한 effective_tier
  email: string;
  iat: number;
  exp: number; // 발급 시각에서 15분 뒤
  jti: string;
}

// 리프레시 토큰 클레임. 30일짜리이며 갱신 엔드포인트에서만 검증한다. 시각은 epoch 초다.
export interface RefreshTokenClaims {
  sub: string;
  typ: "refresh"; // 액세스 토큰과 구분한다. 액세스 토큰에는 typ가 없다.
  iat: number;
  exp: number; // 발급 시각에서 30일 뒤
  jti: string; // 블랙리스트 키다. 갱신 시점에만 검사한다.
}
