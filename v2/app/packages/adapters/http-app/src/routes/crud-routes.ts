...
// C2.1은 user_id를 x-user-id 헤더로 수용한다(DEV 경로). TODO(C2.2): JWT sub에서 파생하는 미들웨어로 교체.
...
function userIdOf(c: { req: { header(name: string): string | undefined } }): string | null {
...
export function registerCrudRoutes(app: Hono, repos: Repositories): void {
  // ── 세션 ──────────────────────────────────────────────
