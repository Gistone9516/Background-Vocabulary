...
const source = new Map(); // key -> "v1" | "authored" | "draft"
...
const counts = { v1: 0, authored: 0, draft: 0 };
for (const f of source.values()) counts[f] += 1;

const deadStillPresent = [...DEAD].filter((k) => v2.has(k)).length;
console.log(
  `대상 키 ${expected} (v2 ${v2.size}, 그중 삭제 대상 ${deadStillPresent}건이 아직 남아 있음)`
);
console.log(`  v1 이식분      ${counts.v1}   (A ${bucket.A.length} + C ${bucket.C.length})`);
console.log(`  AI 작성분      ${counts.authored + counts.draft}   (이 스크립트 ${counts.authored} + 스펙 초안표 ${counts.draft})`);
console.log(`  채운 키 합계   ${source.size}`);
...
if (source.size !== expected) {
  console.error(`\n채운 키 ${source.size} != 대상 ${expected}. 스크립트를 의심할 것.`);
  process.exit(1);
}
...
const aiKeys = [...source.entries()].filter(([, f]) => f !== "v1").map(([k]) => k).sort();
...
function emit(loc) {
  ...
  const body = [...table[loc].entries()].map(([k, v]) => {
    const tag = source.get(k) === "v1" ? "[v1]" : "[AI]";
