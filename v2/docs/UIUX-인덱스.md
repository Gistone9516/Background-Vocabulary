...
| 범위 | 표면 15 · 요소 행 171 · 슬롯 6 · 불변식 26 · 드리프트 13(D-1·D-11·D-13 해소) |
| 코드 기준 | 커밋 `d9fe383` 시점 |
...
| **D-11** | ~~`.sbSearch` 는 정의가 없다~~ **해소(C5-S3c, 2026-08-18).** `shell.css` 에 규칙 신설 | 전수 grep + 실측 | **이 건이 역방향 게이트를 만든 계기다.** 관계 인덱스에 "className 에 쓰였는데 정의가 없다" 검사를 더했고, 이 건 둘을 정확히 잡고 205개 파일에서 거짓 양성 0건이었다. 고친 뒤 조용해지는 것까지 확인했다 |
...
**기존 게이트의 사각지대 — 한 방향뿐이다** `[2026-08-18 정정]`. 이 문단은 처음에 "관계 인덱스가 CSS 클래스에 `readers=` 를 안 붙이므로 죽은 CSS 는 어떤 게이트에도 안 걸린다"고 적었다. **틀렸다.** 관계 인덱스에 `## css-classes-unused` 절이 이미 있고(132건), 머리말이 "기준선이지 결함 목록이 아니다 … 쓸모는 diff 에 있다"라고 성격까지 밝히고 있다. 정의됐는데 안 쓰이는 방향은 **덮여 있다** — D-3·D-4·D-9·D-12·D-13 은 게이트가 못 잡은 것이 아니라 **기준선에 올라와 있는데 아무도 안 읽은 것**이다. 실제로 비어 있는 것은 반대 방향이다: **쓰이는데 정의가 없는 클래스**(D-11 `.sbSearch`)를 잡는 검사가 없다. D-1(`.drawerWrap`)은 양쪽 다 아니다 — 정의도 있고 쓰이기도 하는데 **의미가 틀린** 경우라 정적 검사로는 못 잡는다.
