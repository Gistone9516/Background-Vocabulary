...
- **요금제·비용 차등:** freemium(유료 pro 월 3,500원). 출력량·기능으로 차등(모델은 양 티어 flash 동일). 무료: 좁히기 3턴·총 어휘 8개·상세 3회/세션·그룹별 +2 생성·주 7회 추천. 유료: 좁히기 8턴·총 어휘 32개·상세 무제한·"더 보기"·그룹별 +4 생성·**조건 재탐색**(검색창에 조건 입력하면 아키네이터 재진입, 기존 리스트에 추가)·"AI로 더 정리"(/summarize, 방향성 입력 + 상세 브리핑 출력)·**파일 첨부**(붙여넣은 문서=context_object) 해금(무료는 첨부 버튼 없음, 드롭 시 페이월). 출처 RAG는 양 티어 동일. 전 LLM 호출 `max_tokens` 상한 + 전역 일일 캡 + **누적 어휘 캡(`maxTotal`)** 강제.
