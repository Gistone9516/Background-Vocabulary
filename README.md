...
**현재 단계:** buildflow 구현 진행 중 — **태그→Keep · 로컬 라이브 통합 · 비용/티어 차등 · 이름(배경노트/Vock note) · 다국어(ko·en·ja·zh: 영어 프롬프트 + 출력 로케일 + UI i18n)까지 완료·검증**. 다음 = 보안 하드닝(#5) · 프로덕션 배포(`배경어휘-사이드탭-배포-가이드.md`).
...
- **요금제·비용 차등:** freemium. 출력량·기능으로 차등(모델은 양 티어 flash 동일). 무료: 좁히기 3턴·어휘 4개·상세 3회/세션·주 7회 추천. 유료: 좁히기 8턴·어휘 8개·상세 무제한·"더 보기"·"더 깊이"·"AI로 더 정리"(/summarize) 해금. 출처 RAG는 양 티어 동일. 전 LLM 호출에 `max_tokens` 안전 상한 + 빌드 폭주 방지용 전역 일일 캡.
